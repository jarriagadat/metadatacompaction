...
# Why this exists
Academic PDF libraries usually start chaotic: inconsistent names, missing metadata, publisher junk.

MetadataCompaction automates most of the cleanup so the library becomes immediately usable, leaving only a small number of edge cases for manual renaming when (and if) you need them.
